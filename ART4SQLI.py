...
def foo(f, d, p, name):
    ...
    for t in range(100):
        ...
        # init data queue
        data = []
        def min_distance(feature):

            nblocked = np.asarray(blocked)
            nfeature = np.asarray(feature)
            distnaces = [sim2(b , nfeature) for b in nblocked]
            

            return min(distnaces)
...
with concurrent.futures.ThreadPoolExecutor() as executor:
    for i in range(0, 1):
        executor.submit(foo, f, d, i, "art4sqli_nx_cookie")
